    ...
    def _collect(self, pairs: Dict[str, Any]) -> io.StringIO:
        ...
        for key in pairs:
            ...
            if isinstance(val, str):
                ...
                val = 1 if val else 0
                buffer.write(f"# HELP {self.prefix}_{key}_bool Auto-generated, sorry\n")
                buffer.write(f"# TYPE {self.prefix}_{key}_bool gauge\n")
                buffer.write(f"{self.prefix}_{key}_bool {val}\n")
            elif isinstance(val, datetime):
                val = val.timestamp()
                buffer.write(f"# HELP {self.prefix}_{key}_timestamp_seconds Auto-generated, sorry\n")
                buffer.write(f"# TYPE {self.prefix}_{key}_timestamp_seconds gauge\n")
                buffer.write(f"{self.prefix}_{key}_timestamp_seconds {val}\n")
            elif isinstance(val, dict):
                if 'secret_id_ttl' in val:
                    val = val['secret_id_ttl']
                    buffer.write(f"# HELP {self.prefix}_{key}_timestamp_seconds Time until the secret_id expires in seconds\n")
                    buffer.write(f"# TYPE {self.prefix}_{key}_timestamp_seconds gauge\n")
                    buffer.write(f"{self.prefix}_{'secret_id_ttl'}_timestamp_seconds {val}\n")
            else:
                buffer.write(f"# HELP {self.prefix}_{key}_total Auto-generated, sorry\n")
...
class VaultClient:
    ...
    def get_secret_id_accessors(self, role_name: str) -> List[str]:
        url = urllib.parse.urljoin(self._vault_address, f"v1/auth/{self._mount_path}/role/{role_name}/secret-id?list=true")
    ...
    def destroy_secret_id_accessor(self, role_name: str, secret_id_accessor: str) -> bool:
        url = urllib.parse.urljoin(self._vault_address, f"v1/auth/{self._mount_path}/role/{role_name}/secret-id-accessor/destroy")
        data = {
            "secret_id_accessor": secret_id_accessor,
        }
        resp = requests.post(url=url, data=data, headers={TOKEN_HEADER: self._get_vault_token()})
        if resp.ok:
            return True
        raise VaultException(resp.status_code, url, resp.text)

    def destroy_secret_id(self, role_name: str, secret_id: str) -> bool:
        url = urllib.parse.urljoin(self._vault_address, f"v1/auth/{self._mount_path}/role/{role_name}/secret-id/destroy")
        data = {
            "secret_id": secret_id,
        }
        resp = requests.post(url=url, data=data, headers={TOKEN_HEADER: self._get_vault_token()})
    ...
    def list_role_names(self) -> List[str]:
        url = urllib.parse.urljoin(self._vault_address, f"v1/auth/{self._mount_path}/role?list=true")
        resp = requests.get(url=url, headers={TOKEN_HEADER: self._get_vault_token()})
        if resp.ok:
            return resp.json()["data"]["keys"]
    ...
    def get_role_id(self, role_name: str) -> Optional[str]:
        url = urllib.parse.urljoin(self._vault_address, f"v1/auth/{self._mount_path}/role/{role_name}/role-id")
        resp = requests.get(url=url, headers={TOKEN_HEADER: self._get_vault_token()})
        if resp.ok:
            return resp.json()["data"]["role_id"]
        raise VaultException(resp.status_code, url, resp.text)

    def set_custom_secret_id(self, role_name: str, secret_id: str = None, cidrs: List[str] = None, metadata: Dict[str, Any] = None) -> Dict:
    ...
    def lookup_secret_id_accessor(self, role_name: str, secret_id_accessor: str) -> Dict[str, Any]:
        url = urllib.parse.urljoin(self._vault_address, f"v1/auth/{self._mount_path}/role/{role_name}/secret-id-accessor/lookup")
        data = {
            "secret_id_accessor": secret_id_accessor
        }
        resp = requests.post(url=url, headers={TOKEN_HEADER: self._get_vault_token()}, data=data)
    ...
    def lookup_secret_id(self, role_name: str, secret_id: str) -> Dict[str, Any]:
        url = urllib.parse.urljoin(self._vault_address, f"v1/auth/{self._mount_path}/role/{role_name}/secret-id/lookup")
        data = {
            "secret_id": secret_id
        }
        resp = requests.post(url=url, headers={TOKEN_HEADER: self._get_vault_token()}, data=data)
    ...
    def rotate_secret_id(self, role_id: str, secret_id: str, role_name: str = None, max_age_s: int = None, min_lifetime_s: int = None, percentage: int = None) -> Dict:
        ...
        if self.rotation_strategy.rotate(creation_time, expiration_time):
            ...
            ret["response"] = self.set_custom_secret_id(role_name, secret_id, cidrs=cidrs, metadata=metadata)
...
class ParsingUtils:
    ...
    def get_role_id(args: argparse.Namespace) -> str:
        if args.role_id_json_file:
            if not Utils.is_secure_file(args.role_id_json_file):
                logging.warning("Permissions of file '%s' too liberal, consider setting more restrictive file permissions", args.role_id_json_file)
            role_id = Utils.extract_value_from_json_file(args.role_id_json_path, args.role_id_json_file)
            logging.info("Read role_id '%s' from file '%s'", role_id, args.role_id_json_file)
        else:
    ...
    def get_role_name(args: argparse.Namespace) -> str:
        if args.role_name_json_file:
            if not Utils.is_secure_file(args.role_name_json_file):
                logging.warning("Permissions of file '%s' too liberal, consider setting more restrictive file permissions", args.role_name_json_file)
            role_name = Utils.extract_value_from_json_file(args.role_name_json_path, args.role_name_json_file)
            logging.info("Read role_name '%s' from file '%s'", role_name, args.role_name_json_file)
        else:
    ...
    def parse_args() -> argparse.Namespace:
        ...
        if group_args.config:
            try:
                with open(group_args.config) as cf:
                    config = json.load(cf)
        ...
        ################################################################################################################
        list_secret_accessor_ids = command_subparsers.add_parser(CMD_LIST_SECRET_ID_ACCESSORS, help="List all secret_id_accessors for a role")
        list_secret_accessor_ids.add_argument('-r', '--role-name', required=True)

        ################################################################################################################
        # destroy-secret-accessor-id
        ################################################################################################################
        destroy_secret_accessor_id = command_subparsers.add_parser(CMD_DESTROY_SECRET_ACCESSOR_ID, help="Destroy a secret_id_accessor for a given role_name")
        ...
        ################################################################################################################
        destroy_all_secret_accessor_ids = command_subparsers.add_parser(CMD_DESTROY_ALL_SECRET_ACCESSOR_ID, help="Destroy all secret_id_accessors for a given role_name")
        ...
        group.add_argument('--role-id', help="The role id to add the secret_id to")
        group.add_argument('--role-id-json-file', help="The role id to add the secret_id to")
        group.set_defaults(**config_values)

        group = add_secret_id.add_mutually_exclusive_group(required=False)
        group.add_argument('--secret-id', help="The secret_id to add. If not specified, it is auto-generated.")
        group.add_argument('--secret-id-file', help="The secret_id to add, read from a file. If not specified, it is auto-generated.")
        ...
        rotate_secret_id.add_argument('--role-name-json-path', default=".role_name")
        rotate_secret_id.add_argument('--role-id-json-path', default=".role_id")
...
class Utils:
    ...
    @staticmethod
    def write_secret_id_to_file(secret_id: str, secret_id_file: str) -> None:
        Path(secret_id_file).expanduser().write_text(secret_id)

    @staticmethod
    def upsert_secret_id(secret_id: str, secret_id_json_file: str, json_path: str = ".secret_id") -> None:
        valid_json = False
        if Path(secret_id_json_file).expanduser().exists():
            try:
                json.loads(secret_id_json_file)
                valid_json = True
            except json.decoder.JSONDecodeError as err:
                pass
        content = {}
        if valid_json:
            content = json.loads(secret_id_json_file)
        value = content
        for k in json_path.lstrip(".").split('.'):
            value = value[k]
        Path(secret_id_json_file).expanduser().write_text(json.dumps(value))
    ...
    def read_secret_id(secret_id_file: str) -> str:
        ...
        raise ValueError(f"Can not read secret-id from non-existent file '{secret_id_file.name}'")
...
def _log_rotation(rotated: bool, creation_time: datetime, expiration_time: datetime) -> None:
    ...
    if rotated:
        logging.info("secret_id has been rotated, creation_time: '%s', expiration_time: '%s'%s", creation_time, expiration_time, expiry_str)
    else:
        logging.info("No secret_id rotation needed, creation: '%s', expiration: '%s'%s", creation_time, expiration_time, expiry_str)
...
def run_add_secret_id_subcmd(vault_client: VaultClient, args: argparse.Namespace, json_output: JsonOutput) -> None:
    ...
    if not role_id:
        ...
        logging.info("Fetched role_id for role name %s", role_name)

    if args.destroy_others:
        accessors = vault_client.get_secret_id_accessors(role_name)
        if accessors:
            logging.info("Destroying other secret_id_accessors for role name %s (role_id %s)", role_name, role_id)
    ...
    ret = vault_client.set_custom_secret_id(role_name, secret_id, cidr)
...
def process_new_secret_id(ret: Dict[str, Any], args: argparse.Namespace) -> None:
    """ Decide how to process the newly created secret_id. Either write it to a json or flat file or print it. """
    if args.secret_id_file:
        Utils.write_secret_id_to_file(ret["response"]["secret_id"], args.secret_id_file)
        del ret["response"]["secret_id"]
        logging.info("Wrote new secret_id to file '%s'", args.secret_id_file)
    elif args.secret_id_json_file:
        Utils.upsert_secret_id(ret["response"]["secret_id"], args.secret_id_json_file, args.secret_is_json_path)
    else:
        logging.warning("Printing sensitive data is disregarded, consider writing it to a file.")
        logging.info("New secret_id is: %s", ret["response"]["secret_id"])
